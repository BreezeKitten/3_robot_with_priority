...
resTH = PI/15
LOG_DIR = 'logs/Residual_Learning_midLayer/Reward_400_200_200_100_tanh_none'
SOME_TAG = '_gamma_0.9'
...
test_num = 1
Network_Path = '2_to_3_robot_network/Residual_network_midLayer/Reward_400_200_200_100_tanh_none'
...
def Predict_action_value(agent1, agent2, agent3, V_pred, W_pred):
    ...
    value_matrix = sess.run(Final_predict_value, feed_dict={state_12: state_pred12, state_13: state_pred13})
...
def DL_process(DL_database):
    ...
    two_to_three_network_saver.save(sess, Network_Path+'/test.ckpt')  
    for training_eposide in range(training_eposide_num):
        training_data = Sample_data(data, training_num)
        training_state12, training_state13, training_value = Divide_state_value(training_data)
        sess.run(train_step, feed_dict={state_12: training_state12, state_13: training_state13, value: training_value})
        if training_eposide%100 == 0:
            rs = sess.run(loss_record, feed_dict = {state_12: training_state12, state_13: training_state13, value: training_value})
...
if __name__ == '__main__':
    ...
    with tf.name_scope('2_to_3_NW'):
        ...
        predict_value, mWf, mBf = add_layer(mH4, m_layer4_output_number, 1, 'mWf', 'mBf', activation_function=tf.nn.tanh)
        two_to_three_network_saver = tf.train.Saver({'mW1':mW1,'mW2':mW2,'mW3':mW3,'mW4':mW4,'mWf':mWf,
                            'mB1':mB1,'mB2':mB2,'mB3':mB3,'mB4':mB4,'mBf':mBf})
   
    Final_predict_value = predict_value + nearest_value
    ...
    two_network_saver_12.restore(sess,'2_robot_network/20200226_86p/test.ckpt')
    two_network_saver_13.restore(sess,'2_robot_network/20200226_86p/test.ckpt')
    
    two_to_three_network_saver.restore(sess, Network_Path+'/test.ckpt')
